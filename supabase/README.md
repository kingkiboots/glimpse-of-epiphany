...
**그리고 배포와 별개로 스케줄 등록이 필요합니다.** 등록하지 않으면 DB 행은 사라지는데
Storage 파일이 계속 쌓입니다. 자동화되지 않는 1회성 설정입니다.
...
**주기를 1분이 아니라 5분으로 둔 이유**는, 행 삭제(`delete-expired-exhibits`)가 따로 돌면서
"2시간 뒤 사라진다"는 약속을 이미 지키고 있기 때문입니다. 운영자가 지운 것도 Realtime으로
화면에서 곧바로 내려갑니다. 파일 정리는 그 뒤를 따라오는 청소라서 늦어도 화면에 영향이
없습니다. 1분으로 두면 대부분 지울 것이 없는 헛수고만 늘어납니다.
...
| 크론 | `delete-expired-exhibits` | 10분마다 2시간 지난 행을 삭제 |
...
1. 올린 지 2시간이 지나 `delete-expired-exhibits` 크론이 지움 (순수 SQL)
...
"2시간 뒤 사라진다"는 참가자와의 약속이 깨지지 않아야 하기 때문입니다.
함수가 죽으면 파일 정리만 밀리고 화면에서 사라지는 것은 보장됩니다.

**만료 기준을 자정이 아니라 "올린 지 2시간"으로 둔 것은 타임존 때문입니다.** pg_cron은
UTC로 돕니다. 한국 자정에 맞추려면 스케줄을 `0 15 * * *`로 밀고 삭제 조건도
`at time zone 'Asia/Seoul'`로 날짜를 끊어야 하는데, 여기서 실수하면 행사 도중에 사진이
통째로 날아갑니다. `interval '2 hours'`는 타임존이 개입하지 않아 그 위험 자체가 없습니다.

정책이 두 번 바뀐 흔적이 마이그레이션에 남아 있습니다. 5분(`20260802000100`) →
없음(`20260804000000`) → 2시간(`20260804000100`) 순서입니다.
...
### 업로드가 413 `The object exceeded the maximum allowed size`로 막힌다

**카카오톡 인앱 브라우저(iOS)에서 처음 드러났습니다.** 캔버스가 webp로 **인코딩**하지
못하는 환경이 원인입니다. webp를 화면에 그리는 것과는 다른 이야기이고,
`toDataURL("image/webp")`는 지원하지 않아도 오류를 내지 않고 명세대로 **조용히 PNG를**
돌려줍니다. PNG는 무손실이라 quality가 먹지 않아 300KB로 줄일 방법이 없습니다.

여기에 두 가지가 겹쳤습니다. `browser-image-compression`의 `maxSizeMB`는 보장이 아니라
시도라서 **최대 10회만 줄여보고 목표에 못 미쳐도 그대로 반환**하고, 앱은 그 결과에
`image/webp` 딱지를 덮어씌워 올렸습니다. 그래서 버킷의 mime 검사는 통과하고
`file_size_limit`에서 터졌습니다.

지금은 버킷이 **webp와 jpeg를 모두 받고**(`20260807000000_allow_jpeg_uploads.sql`),
앱이 webp 인코딩 가능 여부를 먼저 확인해 안 되면 jpeg로 갑니다. 압축 결과의 **크기와
실제 타입도 업로드 전에 확인**합니다. 허용 목록은 마이그레이션과 `packages/api`의
`UPLOAD_IMAGE_TYPES`가 **함께** 바뀌어야 합니다.
