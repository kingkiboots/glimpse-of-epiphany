...
- **[예외]:** `projector-web`의 사진 벽은 이 금지의 대상이 아닙니다. 칸이 불규칙하게 흩어진 배치라 Flex/Grid로 재현할 수 없고, 고정 비율 스크린(16:9) 한 대만 대응하면 되므로 좌표 테이블 + 절대 배치가 정답입니다. 아래 "5. 도메인 규칙" 참고.
...
코드를 읽어서는 알 수 없고, 어기면 기획 의도가 깨지는 것들입니다.

**전시물의 수명은 2시간입니다.** 올린 지 2시간이 지나면 `delete-expired-exhibits` 크론이 10분마다 돌며 지웁니다. 자정 기준이 아니라 상대 시간인 이유는 pg_cron이 UTC로 돌기 때문입니다 — 자정에 맞추려다 실수하면 행사 도중에 사진이 날아갑니다. 값은 `EXHIBIT_TTL_HOURS`와 마이그레이션의 `interval`이 **함께** 바뀌어야 합니다. 화면 문구가 이 상수를 그대로 쓰므로, 어긋나면 참가자에게 거짓말이 됩니다.

**감사 메시지는 프로젝터에 표시하지 않습니다.** 스크린에는 사진만 뜹니다. 메시지는 미리보기·운영자 화면과, 참가자가 폰에 간직하는 저장 이미지에만 들어갑니다.

**`projector-web`의 전시 규칙**은 [`apps/projector-web/README.md`](apps/projector-web/README.md)에 상세히 적혀 있습니다. 프로젝터 작업 전에 반드시 읽으십시오. 핵심만 옮기면:

- 칸 18개(설정에서 변경 가능)를 위에서부터 채웁니다.
- 새 사진의 자리는 **"빈 칸 중 가장 위, 없으면 가장 오래된 칸"** 한 규칙으로 정합니다. 19번째가 1번 자리를 덮는 것도, 운영자 삭제로 생긴 구멍을 다음 업로드가 메우는 것도 이 규칙에서 자동으로 나옵니다.
- 운영자 삭제로 생긴 빈칸은 **뒤 사진을 당겨 메우지 않습니다.** 검은 배경 그대로 둡니다.
- 슬라이딩은 **9칸부터** 시작하고, 정지 지점은 칸 단위가 아니라 묶음 단위(12·16·18번 칸 아래)로 끊깁니다.
- 새로고침 시 자리 배치가 달라지는 것은 **의도된 동작**입니다. 순번을 DB에 저장해 보존하려 들지 마십시오.
