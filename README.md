...
수련회 참가자가 자기 폰으로 사진 한 장을 올리면,
그것이 실시간으로 빔프로젝터 화면에 떠오르는 웹 애플리케이션입니다.

함께 적은 감사의 문장은 화면에 띄우지 않고, 참가자가 간직할 이미지를 만드는 데 씁니다.

올라온 사진은 **2시간 뒤 DB와 CDN 양쪽에서 흔적 없이 사라집니다.**
운영자가 그 전에 내릴 수도 있습니다. 어느 쪽이든 되돌릴 수 있는 삭제는 두지 않았습니다.
...
| 150명이 동시에 몰린다 | 클라이언트에서 이미지를 500KB 이하로 압축해 올림 |
| 부적절한 사진이 올라올 수 있다 | 운영자 전용 앱에서 즉시 삭제 |
| 개인 사진이 서버에 남으면 안 된다 | 2시간 뒤 DB와 CDN 양쪽에서 하드 삭제. 되돌릴 수 없다 |

### 세 개의 앱

| 앱 | 사용자 | 하는 일 |
| --- | --- | --- |
| [`apps/mobile-web`](apps/mobile-web) | 참가자 (150명) | 사진 선택 → 감사 작성 → 전시 / 간직할 이미지 저장 |
...
sequenceDiagram
    ...
    Note over M: 준비 화면 (최소 5초)<br/>성구를 읽는 동안<br/>변환 수행
    M->>M: 1920px webp 압축<br/>(webp 인코딩 불가 시 jpeg)

    U->>M: 감사 메시지 작성 (100자)

    opt 간직하기 (서버를 거치지 않음)
        U->>M: "저장하기"
        M->>M: 배경 + 카드를 Canvas로 합성
        M->>U: 공유 시트 → 사진첩에 저장
    end
    ...
    M->>S: 이미지 업로드
    S-->>M: 성공
    M->>D: INSERT exhibits (사진 경로 + 메시지)
    D-->>M: 성공
    M->>U: 완료 화면 "2시간 뒤 사라집니다"

    D-)P: Realtime INSERT 이벤트
    P->>P: 사진만 화면에 렌더링<br/>(메시지는 표시하지 않음)
```
...
**"저장하기"는 서버를 전혀 거치지 않습니다.** 배경과 카드를 폰 안에서 Canvas로 합성해 곧바로 공유 시트로 넘깁니다. 전시와 독립적이라, 올리지 않고 간직만 하는 것도 가능합니다. 웹에는 사진첩에 직접 쓰는 API가 없어 공유 시트를 한 번 거치는 것이 사진첩에 닿는 유일한 경로입니다. 자세한 제약은 [`apps/mobile-web/README.md`](apps/mobile-web/README.md#저장-기능에서-조심할-것)에 정리해 두었습니다.
...
flowchart TD
    ...
    LIVE -->|"2시간 경과"| CRON["pg_cron<br/>delete_expired_exhibits<br/>(10분마다)"]
...
**Storage 파일을 SQL로 지울 수 없기 때문입니다.** `storage.objects`에서 행을 지워도 실제 파일은 남습니다. 파일을 지우려면 service_role 키로 Storage API를 호출해야 하고, 그 키를 안전하게 둘 수 있는 곳은 서버에서만 도는 Edge Function뿐입니다.

그래서 행 삭제는 **순수 SQL 크론**이, 파일 삭제는 **Edge Function**이 맡습니다. Edge Function이 배포되지 않았거나 죽어 있어도 행 삭제는 DB 안에서 돌기 때문에 **"2시간 뒤 사라진다"는 참가자와의 약속은 지켜집니다.** 함수가 멈추면 눈에 보이지 않는 파일 정리만 밀립니다. 반대로 했다면 함수 장애가 곧 개인 사진 노출로 이어졌을 겁니다.

### 왜 자정이 아니라 "올린 지 2시간"인가

**타임존을 끌어들이지 않기 위해서입니다.** Supabase의 pg_cron은 UTC로 돕니다. 한국 자정에 맞추려면 스케줄을 15시로 밀고 삭제 조건도 `Asia/Seoul` 기준으로 날짜를 끊어야 하는데, 여기서 한 번 실수하면 **행사 도중에 사진이 통째로 날아갑니다.** `interval '2 hours'`는 타임존이 개입하지 않아 그 위험 자체가 없습니다.

전시가 2시간 반이라 **초반에 올린 사진은 행사가 끝나기 전에 사라집니다.** 다만 프로젝터 자리가 18칸뿐이라, 업로드가 계속 이어지는 한 그 사진들은 이미 뒤 사진에 밀려나 화면에 없을 시점입니다. 행사가 끝나면 자정까지 기다릴 것 없이 30분 안에 전부 정리됩니다.
...
| `image_path` | `text` `unique` | Storage 내부 경로 (`<uuid>.webp` 또는 `.jpg`). 공개 URL은 클라이언트가 조합 |
| `message` | `text` | 감사 메시지. 입력 UI 100자, DB 제약 200자. **프로젝터에는 표시하지 않는다.** 참가자가 간직할 이미지 합성과 운영자 화면에서만 쓴다 |
| `client_id` | `uuid` `null` | 업로드한 기기 식별자. 반복 업로더를 기기 단위로 정리하기 위한 값 |
| `created_at` | `timestamptz` | 만료 판단 기준. 최신순 정렬과 운영자 화면의 업로드 시각도 겸한다 |
...
| 항목 | 값 | 근거 |
| --- | --- | --- |
| 포맷 | webp (불가 시 jpeg) | 같은 화질에서 JPEG보다 작음. 아래 참고 |
| 목표 크기 | 500KB | 150장 올려도 75MB |
...
**목표 크기는 보장이 아닙니다.** 라이브러리는 최대 10회만 줄여보고 목표에 못 미쳐도 오류 없이 그대로 반환합니다. 그래서 앱이 결과의 **크기와 실제 MIME 타입을 업로드 전에 다시 확인**하고, 넘치면 해상도를 낮춰 한 번 더 시도한 뒤 그래도 안 되면 참가자에게 알립니다.

**jpeg 대비책이 필요한 이유**가 있습니다. 일부 인앱 브라우저(카카오톡 등)는 캔버스가 webp로 **인코딩**하지 못합니다. 그런 환경에서 `toDataURL("image/webp")`는 오류를 내지 않고 명세대로 조용히 PNG를 돌려주는데, PNG는 무손실이라 압축이 통하지 않아 업로드가 통째로 막힙니다. 참가자에게 링크를 카카오톡으로 뿌리는 행사라 이 경로가 곧 주 경로입니다. 앱은 인코딩 가능 여부를 먼저 확인해 안 되면 jpeg로 갑니다.

### 저장과 서빙

공개 버킷에 저장하고 CDN 공개 URL로 바로 서빙합니다. 프로젝터가 빠르게 로드해야 하므로 서명 URL을 쓰지 않습니다. 버킷 자체에 **webp·jpeg만 허용, 파일당 1MB 제한**을 걸어 클라이언트 압축이 실패해도 서버에서 한 번 더 걸러집니다. 허용 목록은 마이그레이션과 `packages/api`의 `UPLOAD_IMAGE_TYPES`가 함께 바뀌어야 합니다.

**캐시 수명은 5분으로 짧게 잡았습니다.** 전시 수명(2시간)과 일부러 다르게 뒀습니다. 캐시를 길게 잡으면 운영자가 내린 사진이 그 뒤에도 CDN 엣지에서 계속 서빙되어 하드 삭제가 성립하지 않습니다. 부적절한 사진을 즉시 내리는 것이 운영자 화면의 존재 이유라, 그 즉시성을 캐시가 갉아먹지 않도록 맞춘 값입니다.
