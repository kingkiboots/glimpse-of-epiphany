...
올라온 사진은 **운영자가 내리기 전까지 화면에 남습니다.**
지우면 DB와 CDN 양쪽에서 흔적 없이 사라집니다. 되돌릴 수 있는 삭제는 두지 않았습니다.
...
| 개인 사진이 서버에 남으면 안 된다 | 삭제는 DB와 CDN 양쪽에서 하드 삭제. 되돌릴 수 없다 |
...
그래서 운영자가 삭제를 누르면 **DB 행만 즉시 사라지고**, 파일은 몇 분 뒤 정리됩니다. 화면에서 내려가는 것은 Realtime DELETE 이벤트를 받는 즉시라 지연이 없습니다. 남는 것은 URL을 이미 알고 있는 사람만 접근할 수 있는 파일 하나이고, 이것도 곧 지워집니다.

**Edge Function이 죽어 있어도 화면에서 내려가는 것은 보장됩니다.** 함수가 멈추면 눈에 보이지 않는 파일 정리만 밀립니다.
...
| `created_at` | `timestamptz` | 최신순 정렬 기준. 운영자 화면의 업로드 시각 |
...
**캐시 수명은 5분으로 짧게 잡았습니다.** 사진은 운영자가 내리기 전까지 남지만, 캐시를 길게 잡으면 내린 뒤에도 CDN 엣지에서 한동안 계속 서빙되어 하드 삭제가 성립하지 않습니다. 부적절한 사진을 즉시 내리는 것이 운영자 화면의 존재 이유라, 그 즉시성을 캐시가 갉아먹지 않도록 맞춘 값입니다.
